...
export const getItems = (cart: CartItemWithProduct[]) => {
    return cart.map((item) => {
        return {
            item_id: item.productId,
            item_name: getIntl(item.product.title, 'en') || item.productId,
            affiliation: "Ergonomica Desk Ecommerce",
            currency: "USD",
            item_variant: item.product ? (item.product.stock && item.product.stock > 0 ? "in-stock" : "out-of-stock") : "other",
            price: item.price,
            quantity: item.qty
        }
    })
