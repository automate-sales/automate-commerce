    ...
    <div className="container mx-auto p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {products?.map((product: Product, index: number) => (
          <Item key={index} link={`/products/${product.sku}`} title={getIntl(product.title, params.lang)} price={product.price} id={product.id} image={`${process.env.NEXT_PUBLIC_IMAGE_HOST}/products/${product.images[0]}`}/>
        ))}
      </div>
